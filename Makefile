...
.PHONY: local-logs
local-logs:
	docker-compose -f ./local/docker-compose.yml logs vue-mqtt.local

# バックエンドのコンテナに入る
.PHONY: local-bash
local-bash:
	docker-compose -f ./local/docker-compose.yml exec vue-mqtt.local /bin/bash
...
.PHONY: deploy-logs
deploy-logs:
	docker-compose -f ./deploy/docker-compose.yml logs vue-mqtt.deploy

# コンテナに入る
.PHONY: deploy-bash
deploy-bash:
	docker-compose -f ./deploy/docker-compose.yml exec vue-mqtt.deploy /bin/bash
